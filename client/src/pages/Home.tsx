      ...
      <section className="relative h-[600px] md:h-[700px] flex items-center overflow-hidden">
        ...
        <div className="container relative z-10 text-white">
          <div className="max-w-2xl space-y-6 animate-in fade-in slide-in-from-bottom-10 duration-700">
            ...
            <h1 className="font-heading font-bold text-4xl md:text-6xl leading-tight" style={{textShadow: '0 4px 12px rgba(0,0,0,0.9), 0 2px 4px rgba(0,0,0,0.8)'}}>
              Do More of What You Love in <span className="text-accent">Columbus</span>
            </h1>
            <p className="text-lg md:text-xl text-white leading-relaxed max-w-xl" style={{textShadow: '0 3px 10px rgba(0,0,0,0.9), 0 1px 3px rgba(0,0,0,0.8)'}}>
              ...
              <div className="flex -space-x-2">
                {[1,2,3,4].map(i => (
                  <div key={`avatar-${i}`} className="w-8 h-8 rounded-full bg-gray-200 border-2 border-primary overflow-hidden">
                    <img src={`https://i.pravatar.cc/100?img=${i+10}`} alt={`User ${i}`} loading="lazy" decoding="async" fetchPriority="low" />
                  </div>
                ))}
              </div>
            ...
            {services.slice(0, 6).map((service: any, idx: number) => (
              <div key={`service-${service.id}-${idx}`} className="group relative overflow-hidden rounded-lg shadow-lg hover:shadow-2xl transition-all duration-300 h-full cursor-pointer">
                <div className="aspect-video overflow-hidden bg-muted relative">
                  <img src={service.image} alt={service.title} loading="lazy" decoding="async" fetchPriority="low" className={`w-full h-full object-cover group-hover:scale-110 transition-transform duration-300 ${service.id === 'cryotherapy' ? 'object-top' : ''}`} />
              ...
              <Link key={`location-${loc.id}-${idx}`} href={`/locations/${loc.id}`}>
                <div className="bg-card border rounded-xl overflow-hidden shadow-sm hover:shadow-lg hover:scale-105 transition-all cursor-pointer h-full">
                  <div className="h-48 overflow-hidden">
                    <img src={loc.image} alt={loc.name} loading="lazy" decoding="async" fetchPriority="low" className="w-full h-full object-cover" />
                  </div>
        ...
        <div className="container max-w-3xl text-center space-y-6">
          ...
          <div className="space-y-3 pt-2">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
              <Button asChild variant="outline" className="border-white text-white hover:bg-white/10 font-bold text-xs md:text-sm h-auto py-2 px-3 rounded-lg whitespace-nowrap">
